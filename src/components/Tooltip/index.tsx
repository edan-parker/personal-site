...
const Tooltip = (props: IProps) => {
  ...
  const wrapperSetActive = useCallback(
    (val) => {
      setActive(val);
    },
    [setActive]
  ...
  const contentWithSetActive = (
    <content.type {...content.props} setTooltipActiveState={wrapperSetActive} />
  );

  return (
    <div ref={node} className="tooltip-wrapper" onClick={handleClick}>
      {children}
      {active && (
        <div className={`tooltip-tip bottom`}>{contentWithSetActive}</div>
      )}
    </div>
  );
};
